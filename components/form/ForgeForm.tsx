    ...
    <>
      ...
      <main className="flex-1 max-w-2xl mx-auto w-full px-4 py-10">
        ...
        <form onSubmit={handleSubmit(onSubmit)} onKeyDown={handleKeyDown}>
          <div className={`sketch-card ${step.cardBg} p-6 sm:p-8 mb-6 fade-up`} key={currentStep}>
            <div className="flex items-start gap-3 mb-5">
              <span className="text-4xl">{step.emoji}</span>
              <div>
                <h2
                  className="text-2xl sm:text-3xl text-ink leading-tight"
                  style={{ fontFamily: "'Permanent Marker', cursive" }}
                >
                  {step.title}
                </h2>
                <p className="font-body text-sm text-pencil mt-1">{step.subtitle}</p>
              </div>
            </div>
            ...
            {step.type === "text" && (
              <div>
                ...
                {step.example && (
                  <p className="font-hand text-xs text-pencil mt-2 italic">💡 {step.example}</p>
                )}
              </div>
            )}
            ...
            {step.type === "style" && (
              <StylePicker
                ...
                onChange={(val) => setValue("communicationStyle", val, { shouldValidate: true })}
          ...
          {apiError && (
            <div className="sketch-card bg-card-pink p-4 mb-4 text-center">
              ...
              <button
                ...
                className="font-hand text-xs text-pencil underline mt-1"
              >
                Dismiss
              </button>
            </div>
